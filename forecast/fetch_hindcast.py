...
def main() -> None:
    ...
    ap.add_argument("--product", choices=["nbs", "nbh", "nbe"], default="nbs")
    ...
    got = skip = miss = 0
    for d in dates:
        dest = DEST / f"{args.product}_{d}_{args.cycle}z.txt"
        if dest.exists() and dest.stat().st_size > 1_000_000:
            skip += 1
            continue
        url = f"{BASE}/blend.{d}/{args.cycle}/text/blend_{args.product}tx.t{args.cycle}z"
        try:
